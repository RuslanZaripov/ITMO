...
class BaseTests {
        ...
        val bot = chatBot(testClient) {
            ...
            behaviour {
                into<WithNameContext> {
                    ...
                    onMessageContains("^") {
                        client.sendMessage(message.chatId, "error")
                    }
                ...
                AskNameContext into {
                    onCommand("start") {
                        client.sendMessage(message.chatId, "hi!")
                        setContext(AskNameContext)
                    }

                    onMessageContains("^") {
                        client.sendMessage(message.chatId, "hello")
                    }
    ...
    @Test
    fun testMessageProcessorPredicate() {
        ...
        val bot = chatBot(testClient) {
            ...
            behaviour {
                (IS_ADMIN * IS_EVEN_MESSAGE_ID).into {
                    onCommand("bruh") {
                        client.sendMessage(message.chatId, "Bruh")
                    }
                }

                IS_ADMIN.into {
                    onCommand("ban_user") {
                        client.sendMessage(message.chatId, "User banned")
                    }

                    IS_EVEN_MESSAGE_ID.into {
                        onMessage {
                            client.sendMessage(message.chatId, "Even message id for admin")
                        }
                    }
                }
                ...
                IS_EVEN_MESSAGE_ID.into {
                    onMessage {
                        client.sendMessage(message.chatId, "Even message id for clowns")
                    }
                }
        ...
        bot.sendText("/help")
        assertMessage(1L, "I need help too")

        bot.sendText("/ban_user")
        assertNoMessages()

        bot.sendText(1L, ChatId.Id(316671439L), "/ban_user")
        assertMessage(316671439L, "User banned")

        bot.sendText("test")
        assertNoMessages()

        bot.sendText(2L, ChatId.Id(316671439L), "test")
        assertMessage(316671439L, "Even message id for admin")

        bot.sendText(3L, ChatId.Id(316671439L), "test")
        assertNoMessages()

        bot.sendText("/bruh")
        assertNoMessages()

        bot.sendText(4L, ChatId.Id(316671439L), "/bruh")
        assertMessage(316671439L, "Bruh")
    }
    ...
    private fun ChatBot.sendText(messageId: Long, chatId: ChatId, text: String) {
        processMessages(
            Message(
                id = messageId,
                chatId = chatId,
