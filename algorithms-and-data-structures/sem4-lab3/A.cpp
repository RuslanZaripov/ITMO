...
int dfs(Edge* edge, const int flow, long long& minCost) {
    if (edge == nullptr) {
        return flow;
    }
    int delta = dfs(path[edge->from], std::min(flow, edge->capacity - edge->flow), minCost);
    minCost += edge->weight;
    edge->flow += delta;
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        std::cin >> from >> to >> capacity >> weight;

        from -= 1;
        to -= 1;

        addEdges(from, to, capacity, weight);
    }

    std::cout << findMinCost(n, s, t) << '\n';
}