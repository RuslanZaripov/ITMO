...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val minAccount = acquire(minIndex, this)
            val maxAccount = acquire(maxIndex, this)

            val from = if (fromIndex < toIndex) minAccount else maxAccount
            val to = if (fromIndex < toIndex) maxAccount else minAccount
