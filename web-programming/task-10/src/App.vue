<template>
    <div id="app">
        ...
        <Middle :comments="comments" :posts="posts" :users="users"/>
        <Footer :postsNumber="Object.values(this.posts).length"
                :usersNumber="Object.values(this.users).length"/>
    </div>
</template>
...
export default {
    ...
    beforeCreate: function () {
        ...
        this.$root.$on("onRegister", (name, login) => {
            if (login === "" || name === "") {
                this.$root.$emit("onRegisterValidationError", "Login and name are required");
                return;
            }

            if (login.length < 3 || login.length > 16) {
                this.$root.$emit("onRegisterValidationError", "Required size of login is between 3...16");
                return;
            }
            ...
            if (name.length < 1 || name.length > 32) {
                this.$root.$emit("onRegisterValidationError", "Required size of name is between 1...32");
                return;
            }

            const id = Math.max(...Object.keys(this.users)) + 1;
            this.$root.$set(this.users, id, {id, login, name});
